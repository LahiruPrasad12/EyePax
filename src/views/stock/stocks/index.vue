    ...
    <ion-content fullscreen>
      ...
      <ion-list>
        ...
        <ion-grid style="margin-top: 5%">
          <ion-row>
            <ion-col v-for="stock in all_stock" :key="stock._id" size="6">
              <ion-card
                class="video anim"
                style="--delay: 0.5s; border-color: #0d0d0d"
              >
                <img
                  src="https://i.postimg.cc/QMkZFG1K/layers-animat.gif"
                  type=""
                  v-if="stock.stocks.Availability === true"
                />
                <img
                  src="https://i.postimg.cc/SRGG9dVr/comp-3.gif"
                  type=""
                  v-if="stock.stocks.Availability === false"
                />
                <div class="video-name">
                  {{ stock.stocks.item_code.substring(0, 7) }}
                </div>
                <div class="video-by" v-if="stock.stocks.Availability === true">
                  Status :
                  {{
                    stock.stocks.Availability ? "Available" : "Not Available"
                  }}
                </div>
                <div
                  class="video-by2"
  ...
  methods: {
    async getAllStocks(selected_status) {
      try {
        ...
        let respond = (await StockApis.getAllStocks(selected_status)).data;
        console.log(respond);
